...
| 测试 | pytest | 27 项接口测试 + 32 项线上冒烟脚本（共 59 项全绿） |
...
| Linux 基础 | [黑马 Linux 快速入门（BV1n84y1i7td）](https://www.bilibili.com/video/BV1n84y1i7td) | 常用命令、文件权限、进程 | `start.command` | 会用基本命令 |
...
| 云部署 | [Render 部署](https://render.com/docs) 官方文档 + 回看阶段 5 Docker/Linux（或搜「Render 部署」） | 环境变量、启动命令、健康检查 | `render.yaml` | 能部署出公网链接 |
...
2. 认证：注册（自动建 13 个学生场景默认分类）→ 登录 → JWT；
...
已完成：`screenshots/` 里有 7 张截图（登录页、仪表盘、流水管理、年度报告、AI 记账、AI 智能分类、AI 月度分析），README 已全部引用。
有条件的可以再录一段 1 分钟演示视频。
...
5. 编写 27 项接口测试 + 32 项线上冒烟测试（共 59 项）全部通过；Docker 两阶段构建（Node 打包前端 →
...
> "我做了一个多用户记账系统，从数据库设计、JWT 登录到前端图表、Docker 部署都是自己从 0 写的，59 项自动化测试全过，已经部署上线，公网可以访问。"

### 三、面试怎么讲（STAR）

1. **背景**：想做一个功能完整、能独立讲清的 Python 全栈项目，选记账是因为需求清晰、人人可用；
2. **动作**：先设计 4 张表和 RESTful 接口，再做 JWT 认证，然后写 Vue3 前端，最后补测试和 Docker；
3. **结果**：生活费规划、多钱包、报表、导入导出全部可用，59 项自动化测试，Docker 单容器部署，公网可访问；
...
**问：金额为什么用 Decimal 而不用 float？**
答：float 有二进制精度误差，账会算不平。本项目金额一律用 `Decimal` + `Numeric(10,2)`（数据库精确到分）存储与计算，只在接口 JSON 序列化时转 float，避免把金额存成浮点。
...
4. **金额为什么用 Decimal 而不用 float？**
   答：float 有二进制精度误差，账会算不平；本项目统一用 `Decimal` + `Numeric(10,2)`（精确到分）存储与计算，仅在 JSON 序列化时转 float。
...
13. **为什么写测试？测试覆盖了什么？**
    答：防止改坏旧功能。27 项接口测试覆盖注册/登录/分类/钱包/流水/预算/统计/导出/分页，另有 32 项线上冒烟测试直接对着公网地址跑，共 59 项全绿，CI 里每次提交自动跑。
